...
from wrappers import SkipFrameWrapper
from stable_baselines3 import PPO
from super_parameters import *
from wrappers import CurriculumRLRewardWrapper
import numpy as np
from stable_baselines3.common.monitor import Monitor
import gym
from wrappers import CropObservation

def create_env(seed: int = None, eval_mode: bool = False):
    """
    :param seed: 训练模式下的固定seed
    ...
    if not eval_mode:
        env = CurriculumRLRewardWrapper(env)
        if seed is not None:
            env.seed(seed)
