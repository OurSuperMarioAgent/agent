...
class CropObservation(ObservationWrapper):
    """
    Gym 观测包装器：裁剪图像的指定区域，适配 Mario/Atari 等视觉类环境
    ...
    def __init__(self, env: gym.Env, crop_box: tuple):
        ...
        if len(original_shape) == 3:  # RGB/带通道的灰度图：(H, W, C)
        ...
        elif len(original_shape) == 2:  # 无通道的灰度图：(H, W)
            new_height = self.bottom - self.top
            new_width = self.right - self.left
        ...
        # 4. 更新观测空间（必须！否则模型输入形状不匹配）
        self.observation_space = Box(
            low=original_low.min(),  # 像素值下限（通常 0）
            high=original_high.max(),  # 像素值上限（通常 255）
